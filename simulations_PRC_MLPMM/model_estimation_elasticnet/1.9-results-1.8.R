...
c.list = cens.list = vector('list', n.cases)
...
for (a in 1:n.cases) {
  ...
  # save results
  c.list[[a]] = as.data.frame(c.values)
  auc3.list[[a]] = as.data.frame(auc.values.t3)
  auc5.list[[a]] = as.data.frame(auc.values.t5)
  auc7.list[[a]] = as.data.frame(auc.values.t7)
  cens.list[[a]] = cens.perc
